...
const Transaction = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const endpoint =
        role === "ADMIN" ? "/api/transactions/me" : "/api/transactions/me";
  ...
  useEffect(() => {
    ...
    Promise.all([fetchUsers(), fetchTransactions()]).finally(() =>
      setLoading(false),
    );
  }, [token, role]);
  ...
  return (
    <div className="min-h-screen bg-[#f0f9e8] dark:bg-gray-900 p-6 pt-24 container mx-auto">
      ...
      {transactions.length > 0 ? (
        ...
        <p className="text-center text-gray-500 mt-20">No transactions found</p>
      )}
    </div>
  );
};
