      ...
      <form onSubmit={handleRegister} className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md space-y-4">
        ...
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2">
          ...
          <input type="text" placeholder="Full Name" value={name}
            onChange={(e) => setName(e.target.value)} className="w-full outline-none" />
        </div>
        ...
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2">
          ...
          <input type="text" placeholder="Phone Number" value={phone}
            onChange={(e) => setPhone(e.target.value)} className="w-full outline-none" />
        </div>
        ...
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2">
          ...
          <input type="text" placeholder="Address" value={address}
            onChange={(e) => setAddress(e.target.value)} className="w-full outline-none" />
        </div>
        ...
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2">
          ...
          <input type="text" placeholder="Email" value={email}
            onChange={(e) => setEmail(e.target.value)} className="w-full outline-none" />
        </div>
        ...
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2">
          ...
          <input type={showPassword ? "text" : "password"} placeholder="Password" value={password}
            onChange={(e) => setPassword(e.target.value)} className="w-full outline-none" />
          <span onClick={() => setShowPassword(!showPassword)} className="ml-2 cursor-pointer text-gray-500">
        ...
        <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2">
          ...
          <input type={showConfirm ? "text" : "password"} placeholder="Confirm Password" value={confirmPassword}
            onChange={(e) => setConfirmPassword(e.target.value)} className="w-full outline-none" />
          <span onClick={() => setShowConfirm(!showConfirm)} className="ml-2 cursor-pointer text-gray-500">
        ...
        {showLoginModal && (
          <div className="bg-yellow-50 border border-yellow-300 text-yellow-800 px-4 py-3 rounded text-sm text-center">
            This email is already registered.{" "}
            <Link to="/login" className="font-semibold underline text-green-700">Login instead</Link>
          </div>
        )}
      </form>
    </div>
