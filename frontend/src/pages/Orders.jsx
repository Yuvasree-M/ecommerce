...
import {
  FaTruck, FaCheckCircle, FaBox, FaMapMarkerAlt,
  FaPhone, FaReceipt, FaTimes, FaUndo, FaStar, FaLeaf,
} from "react-icons/fa";
        ...
        <div className="p-5 space-y-4">
          {isReturn && (
            <div className="flex items-start gap-2.5 bg-teal-50 border border-teal-200 rounded-xl px-4 py-3">
              <span className="text-base flex-shrink-0">💳</span>
              <p className="text-xs text-teal-700 font-medium leading-relaxed">
                Once approved by our team, the refund will be automatically credited
                back to your original payment method via Razorpay within <strong>5–7 business days</strong>.
              </p>
            </div>
          )}
...
// ─── Feedback modal (shown for DELIVERED orders) ──────────────────────────────
const FeedbackModal = ({ orderId, feedback, onClose, onSuccess })=> {
...
const [reviewText, setReviewText] = useState(feedback?.reviewText || "");
const [preview, setPreview] = useState(feedback?.imageUrl || null);
  const handleImage = (e) => {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const url = isEdit
      ? `${import.meta.env.VITE_API_URL || ""}/api/feedback/${feedback.id}`
      ...
      >
        ...
        <div className="px-5 py-4 flex items-center justify-between bg-green-50 border-b border-green-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-extrabold text-gray-800">Share Your Experience</h3>
              <p className="text-xs text-gray-400">Your review helps other customers</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            ...
            {rating > 0 && (
              <p className="text-xs text-green-600 font-semibold mt-1 ml-0.5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Tell us about the quality, freshness, packaging…"
              rows={3}
              maxLength={500}
              className="w-full border-2 border-gray-200 focus:border-green-400 rounded-xl px-3 py-2 text-sm text-gray-700 outline-none resize-none transition-colors"
            />
          ...
          <div>
            ...
            {preview ? (
              ...
              <label className="flex items-center gap-2 cursor-pointer w-fit bg-gray-50 hover:bg-green-50 border-2 border-dashed border-gray-200 hover:border-green-300 rounded-xl px-4 py-3 transition-all">
    ...
    {success}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              onClick={handleSubmit}
              disabled={submitting}
              className="flex-1 bg-green-600 hover:bg-green-700 text-white text-sm font-bold py-2.5 rounded-xl transition-all disabled:opacity-60"
            >
             {submitting
  ? "Saving..."
  : feedback
  ...
  (async () => {
    try {
      const [ordersData, feedbackData] = await Promise.all([
        apiFetch("/api/orders"),
        apiFetch("/api/feedback/my")
      ]);

      setOrders(ordersData || []);

      const reviewed = new Set(
        feedbackData.map(f => f.orderId)
      );
...
const handleFeedbackSuccess = async () => {
  const feedbackData = await apiFetch("/api/feedback/my");
  setMyFeedback(feedbackData);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-5">
          {orders.map((order, idx) => {
            ...
            const canReturn = order?.status === "DELIVERED" && within7Days;

            // Show "Leave a Review" button for delivered orders not yet reviewed
            const existingReview = myFeedback.find(
            ...
            return (
              ...
              >
                ...
                <div className="p-5 md:p-6">
                  ...
                  {/* "Leave a Review" nudge for delivered orders */}
                  {canReview && (
                    <div className="mt-4 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                      <div className="flex gap-0.5">
                        {[1, 2, 3, 4, 5].map(s => (
                          <FaStar key={s} size={12} className="text-amber-400" />
                        ))}
                      </div>
                      <p className="text-xs font-medium text-amber-800 flex-1">
                        How was your order? Your review helps others!
                      </p>
                      <button
                        onClick={() => setFeedbackModal({ orderId: order.id })}
  ...
  <div className="mt-4 flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
    ...
    <button
      onClick={() =>
        setFeedbackModal({
          orderId: order.id,
          feedback: existingReview
        })
      }
      className="text-xs font-bold text-white bg-blue-500 hover:bg-blue-600 px-3 py-1.5 rounded-lg"
    >
      Edit Review
    </button>
  </div>
