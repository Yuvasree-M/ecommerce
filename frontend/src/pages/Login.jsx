...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (error.code === "auth/wrong-password") setErrors({ general: "Incorrect password" });
      else if (error.code === "auth/user-not-found") setErrors({ general: "User not found. Register first." });
      else setErrors({ general: "Login failed. Try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (role === "USER") navigate("/products");
  }, [user, role]);
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#f0f9e8] px-4">
      <form onSubmit={handleLogin} className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md space-y-4">
        ...
        <div className="flex justify-center">
          <button type="submit" disabled={loading} className="w-[120px] bg-green-700 text-white py-2 rounded-md hover:bg-green-600">{loading ? "Logging in..." : "Login"}</button>
        </div>
