...
const handleLogout = async () => {
  ...
  // Scroll highlight for landing page
  useEffect(() => {
                ...
                <div className="absolute right-0 mt-3 w-40 bg-gray-800 shadow-lg rounded-xl p-3 space-y-2">
                  {!user ? (
                    ...
                    <>
                      ...
                      <Link to="/transactions" onClick={closeAllMenus} className="block text-sm text-gray-200 hover:text-green-400">Transaction</Link>
                      <button
  ...
  Logout
      ...
      {showScrollUp && (
        <button
          ...
          className="fixed bottom-6 right-6 bg-green-400 hover:bg-green-300 text-gray-900 p-3 rounded-full shadow-lg transition z-50"
        >
          <FaArrowUp />
        </button>
      )}
