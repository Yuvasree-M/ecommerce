...
export const saveOrder = async (req, res) => {
  try {
    ...
    // Save order (without paymentId)
    const orderRef = await db.collection("orders").add({
      userId,
      items: itemsWithDetails,
      address,
      phone,
      paymentType, // only payment method
    ...
    if (paymentType !== "COD") {
      await db.collection("transactions").add({
        userId,
        orderId: orderRef.id,
        transactionId: shortid.generate(), // generate a unique transaction ID
        transactionType: paymentType,      // ONLINE / CARD / UPI
